<?php\n\nreturn array (
  'Dashboard' => 'لوحة التحكم',
  'Employees' => 'الموظفين',
  'Departments' => 'الأقسام',
  'Positions' => 'المناصب',
  'Attendance' => 'الحضور والانصراف',
  'Leave Requests' => 'طلبات الإجازة',
  'Payroll' => 'الرواتب',
  'Performance' => 'التقييم',
  'Settings' => 'الإعدادات',
  'Profile' => 'الملف الشخصي',
  'Logout' => 'تسجيل الخروج',
  'Login' => 'تسجيل الدخول',
  'Register' => 'تسجيل',
  'Home' => 'الرئيسية',
  'Search' => 'بحث...',
  'Language' => 'اللغة',
  'English' => 'الإنجليزية',
  'Arabic' => 'العربية',
  'Name' => 'الاسم',
  'Email' => 'البريد الإلكتروني',
  'Phone' => 'الهاتف',
  'Address' => 'العنوان',
  'Salary' => 'الراتب',
  'Hire Date' => 'تاريخ التوظيف',
  'Status' => 'الحالة',
  'Actions' => 'الإجراءات',
  'Edit' => 'تعديل',
  'Delete' => 'حذف',
  'View' => 'عرض',
  'Create' => 'إنشاء',
  'Update' => 'تحديث',
  'Save' => 'حفظ',
  'Cancel' => 'إلغاء',
  'Back' => 'رجوع',
  'Confirm' => 'تأكيد',
  'Yes' => 'نعم',
  'No' => 'لا',
  'Active' => 'نشط',
  'Inactive' => 'غير نشط',
  'On Leave' => 'في إجازة',
  'Terminated' => 'منتهي',
  'Employee Details' => 'بيانات الموظف',
  'Personal Information' => 'المعلومات الشخصية',
  'Emergency Contact' => 'الاتصال الطارئ',
  'Department' => 'القسم',
  'Position' => 'المنصب',
  'Birth Date' => 'تاريخ الميلاد',
  'National ID' => 'الرقم الوطني',
  'Contact Name' => 'اسم جهة الاتصال',
  'Contact Phone' => 'هاتف جهة الاتصال',
  'Contact Relation' => 'صلة القرابة',
  'Add Employee' => 'إضافة موظف',
  'Edit Employee' => 'تعديل موظف',
  'Employee List' => 'قائمة الموظفين',
  'No employees found' => 'لا يوجد موظفين',
  'Department Name (English)' => 'اسم القسم (إنجليزي)',
  'Department Name (Arabic)' => 'اسم القسم (عربي)',
  'Manager' => 'المدير',
  'Add Department' => 'إضافة قسم',
  'Edit Department' => 'تعديل قسم',
  'Department List' => 'قائمة الأقسام',
  'Position Name (English)' => 'اسم المنصب (إنجليزي)',
  'Position Name (Arabic)' => 'اسم المنصب (عربي)',
  'Add Position' => 'إضافة منصب',
  'Edit Position' => 'تعديل منصب',
  'Position List' => 'قائمة المناصب',
  'Check In' => 'تسجيل الدخول',
  'Check Out' => 'تسجيل الخروج',
  'Date' => 'التاريخ',
  'Check In Time' => 'وقت الدخول',
  'Check Out Time' => 'وقت الخروج',
  'Hours Worked' => 'ساعات العمل',
  'Notes' => 'ملاحظات',
  'Today\'s Attendance' => 'حضور اليوم',
  'Attendance History' => 'سجل الحضور',
  'Leave Type' => 'نوع الإجازة',
  'Start Date' => 'تاريخ البدء',
  'End Date' => 'تاريخ الانتهاء',
  'Reason' => 'السبب',
  'Apply for Leave' => 'تقديم طلب إجازة',
  'Leave Requests List' => 'قائمة طلبات الإجازة',
  'Pending' => 'قيد الانتظار',
  'Approved' => 'موافق عليه',
  'Rejected' => 'مرفوض',
  'Approve' => 'موافقة',
  'Reject' => 'رفض',
  'Vacation' => 'إجازة سنوية',
  'Sick' => 'إجازة مرضية',
  'Unpaid' => 'إجازة بدون أجر',
  'Maternity' => 'إجازة أمومة',
  'Paternity' => 'إجازة أبوة',
  'Rejection Reason' => 'سبب الرفض',
  'Days Count' => 'عدد الأيام',
  'Month' => 'الشهر',
  'Year' => 'السنة',
  'Basic Salary' => 'الراتب الأساسي',
  'Bonuses' => 'الحوافز',
  'Deductions' => 'الخصومات',
  'Net Salary' => 'صافي الراتب',
  'Calculate Payroll' => 'احتساب الراتب',
  'Payroll List' => 'قائمة الرواتب',
  'Generate Payroll' => 'إنشاء الرواتب',
  'Mark as Paid' => 'تحديد كمدفوع',
  'Draft' => 'مسودة',
  'Calculated' => 'محسوب',
  'Paid' => 'مدفوع',
  'Details' => 'التفاصيل',
  'Rating' => 'التقييم',
  'Comments' => 'التعليقات',
  'Review Date' => 'تاريخ التقييم',
  'Reviewer' => 'المقييم',
  'Add Review' => 'إضافة تقييم',
  'Performance Reviews' => 'تقييمات الأداء',
  'Poor' => 'ضعيف',
  'Fair' => 'مقبول',
  'Good' => 'جيد',
  'Very Good' => 'جيد جداً',
  'Excellent' => 'ممتاز',
  'Submit Review' => 'إرسال التقييم',
  'Acknowledged' => 'تم الاطلاع',
  'Total Employees' => 'إجمالي الموظفين',
  'Active Employees' => 'الموظفين النشطين',
  'Present Today' => 'حاضر اليوم',
  'Pending Leave Requests' => 'طلبات الإجازة المعلقة',
  'Total Payroll' => 'إجمالي الرواتب',
  'Welcome' => 'مرحباً',
  'HR Management System' => 'نظام إدارة الموارد البشرية',
  'Login to your account' => 'تسجيل الدخول إلى حسابك',
  'Password' => 'كلمة المرور',
  'Remember Me' => 'تذكرني',
  'Forgot Password' => 'نسيت كلمة المرور',
  'Dashboard Overview' => 'نظرة عامة على لوحة التحكم',
  'are_you_sure' => 'هل أنت متأكد؟',
  'cannot_undo' => 'لا يمكن التراجع عن هذا الإجراء.',
  'success' => 'نجاح',
  'error' => 'خطأ',
  'created_successfully' => 'تم الإنشاء بنجاح',
  'updated_successfully' => 'تم التحديث بنجاح',
  'deleted_successfully' => 'تم الحذف بنجاح',
  'operation_failed' => 'فشل العملية',
  'validation_error' => 'خطأ في التحقق',
  'select_department' => 'اختر القسم',
  'select_position' => 'اختر المنصب',
  'select_status' => 'اختر الحالة',
  'select_employee' => 'اختر الموظف',
  'select_reviewer' => 'اختر المقييم',
  'Admin Dashboard' => 'لوحة تحكم الإدارة',
  'Attendance Report' => 'تقرير الحضور',
  'Payroll Report' => 'تقرير الرواتب',
  'Leave Report' => 'تقرير الإجازات',
  'Total Days' => 'إجمالي الأيام',
  'Average Hours' => 'متوسط الساعات',
  'Present' => 'حاضر',
  'Absent' => 'غائب',
  'Generate' => 'إنشاء',
  'Filter' => 'تصفية',
  'All' => 'الكل',
  'No records found' => 'لا توجد سجلات',
  'Total Bonuses' => 'إجمالي الحوافز',
  'Recent Employees' => 'الموظفين الجدد',
  'Quick Actions' => 'إجراءات سريعة',
  'Employee Reports' => 'تقارير الموظفين',
  'All Reports' => 'جميع التقارير',
  'Today Attendance' => 'حضور اليوم',
  'Total Net Salary' => 'إجمالي صافي الراتب',
  'Employee not found' => 'لم يتم العثور على الموظف',
  'Department not found' => 'لم يتم العثور على القسم',
  'Position not found' => 'لم يتم العثور على المنصب',
  'Attendance not found' => 'لم يتم العثور على الحضور',
  'Leave request not found' => 'لم يتم العثور على طلب الإجازة',
  'Payroll not found' => 'لم يتم العثور على الراتب',
  'Review not found' => 'لم يتم العثور على التقييم',
  'Already checked in today' => 'تم تسجيل الدخول بالفعل اليوم',
  'No check-in found for today' => 'لم يتم العثور على تسجيل دخول لهذا اليوم',
  'Already checked out today' => 'تم تسجيل الخروج بالفعل اليوم',
  'Check in successful' => 'تم تسجيل الدخول بنجاح',
  'Check out successful' => 'تم تسجيل الخروج بنجاح',
  'Attendance generated' => 'تم إنشاء سجل الحضور',
  'Leave request approved' => 'تمت الموافقة على طلب الإجازة',
  'Leave request rejected' => 'تم رفض طلب الإجازة',
  'Payroll generated successfully' => 'تم إنشاء الرواتب بنجاح',
  'Payroll marked as paid' => 'تم تحديد الرواتب كمدفوعة',
);\n